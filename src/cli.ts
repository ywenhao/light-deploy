...
async function zipFiles(config: DeployOptions) {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const outputFilePath = path.resolve(distDir, zipFilename!)
...
async function uploadFile(ssh: NodeSSH, config: DeployOptions) {
  const { distDir, serverWebDir, zipFilename } = config.project

  consola.info(`上传 ${zipFilename!} 至目录 ${config.project.serverWebDir}`)

  try {
    await ssh.putFile(path.resolve(distDir, zipFilename!), path.join(serverWebDir, zipFilename!))
...
async function deleteLocalZipFile(config: DeployOptions) {
  const { distDir, zipFilename } = config.project
